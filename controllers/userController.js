...
// GET /api/user/:userId/target
exports.getTarget = (req, res) => {
  const { userId } = req.params;

  const sql = "SELECT * FROM learning_targets WHERE user_id = ?";
  
  db.get(sql, [userId], (err, row) => {
    if (err) {
      console.error("Error getTarget:", err.message);
      return res.status(500).json({ error: err.message });
    }
    
    if (row) {
      res.json(row); // Kirim target yang ada
    } else {
      // Jika user belum punya target, kirim target default
      res.json({ user_id: userId, target_type: 'modules', target_value: 7 });
    }
  });
};

// POST /api/user/:userId/target
exports.setTarget = (req, res) => {
  const { userId } = req.params;
  // Ambil target_value dari body. Asumsikan target_type selalu 'modules'
  const { target_value } = req.body; 

  const sql = `
    INSERT OR REPLACE INTO learning_targets (user_id, target_type, target_value)
    VALUES (?, 'modules', ?)
  `;
  
  db.run(sql, [userId, target_value], function(err) {
    if (err) {
      console.error("Error setTarget:", err.message);
      return res.status(500).json({ error: err.message });
    }
    console.log(`Target baru untuk ${userId}: ${target_value} modul`);
    res.status(200).json({ user_id: userId, target_type: 'modules', target_value: target_value });
  });
};