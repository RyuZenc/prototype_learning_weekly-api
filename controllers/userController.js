...
function calculateStreak(trackings, userId) {
  // Cek 't.developer_id' ADA SEBELUM membandingkan
...
function getActivityStats(userId) {
  ...
  const modulesThisWeek = db.trackings.filter(
    (t) =>
      t.developer_id && // Cek 't.developer_id' ADA
  ...
  const submissions = db.submissions.filter(
    (s) => s.submitter_id && s.submitter_id == userId // Cek 's.submitter_id' ADA
  ).length;

  const assessments = db.examResults.filter((r) => r.is_passed === '1').length;

  const completedJourneys = db.completions
    .filter((c) => c.user_id && c.user_id == userId) // Cek 'c.user_id' ADA
    .map((c) => c.journey_id);

  const learningHours = db.journeys
    .filter((j) => j.id && completedJourneys.includes(j.id)) // Cek 'j.id' ADA
...
exports.getStreak = (req, res) => {
  ...
  // --- [PERUBAHAN UTAMA] ---
  // Cek 'u.id' ADA SEBELUM membandingkan
